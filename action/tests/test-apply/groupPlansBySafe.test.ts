...
describe('groupPlansBySafe', () => {
  ...
  it('TB-2: single plan → single group of one', () => {
    const plan = makePlan({ safeAddress: '0xaaa', chainId: 1 });
    const groups = groupPlansBySafe([plan]);
    expect(groups).toHaveLength(1);
    expect(groups[0]!.safeAddress).toBe('0xaaa');
  ...
  it('TB-3: two plans, same (safeAddress, chainId) → one group of two', () => {
    const a = makePlan({ safeAddress: '0xaaa', chainId: 1 });
    const b = makePlan({ safeAddress: '0xaaa', chainId: 1 });
  ...
  it('TB-4: same safeAddress but different chainId → two groups', () => {
    const a = makePlan({ safeAddress: '0xaaa', chainId: 1 });
    const b = makePlan({ safeAddress: '0xaaa', chainId: 137 });
    const groups = groupPlansBySafe([a, b]);
  ...
  it('TB-5: same chainId but different safeAddress → two groups', () => {
    const a = makePlan({ safeAddress: '0xaaa', chainId: 1 });
    const b = makePlan({ safeAddress: '0xbbb', chainId: 1 });
    const groups = groupPlansBySafe([a, b]);
    expect(groups).toHaveLength(2);
    expect(groups[0]!.safeAddress).toBe('0xaaa');
    expect(groups[1]!.safeAddress).toBe('0xbbb');
  });

  it('TB-6: order is deterministic — groups appear in first-encounter order, plans preserve input order', () => {
    const a1 = makePlan({ safeAddress: '0xaaa', chainId: 1 });
    const b1 = makePlan({ safeAddress: '0xbbb', chainId: 1 });
    const a2 = makePlan({ safeAddress: '0xaaa', chainId: 1 });
    const b2 = makePlan({ safeAddress: '0xbbb', chainId: 1 });
